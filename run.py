...
def menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            exit_masage = f"{YELLOW}\n Thank you for your time. You can take \
the test at any time convenient for you. See you again!!!{RESET}"
...
def check_previous_score():
    ...
    data_name = input(f"{BLUE}\nEnter your name to get your previous \
test results: {RESET}\n")
    ...
    if data_name in existing_sheets:
        ...
        if len(rows) > 0:
            ...
            print(f"\n{RED}No data found for '{data_name}'. Please check the \
spelling of your name or take the test first.{RESET}")
    else:
        print(f"\n{RED}No data found for '{data_name}'. Please check the \
spelling of your name or take the test first.{RESET}")
...
def info():
    """
    Information about the purpose of psychological testing and instructions
    for taking the test
    """
    welcome_text = (f"\n{BLUE}Welcome to psychological testing.{RESET} \
In this test you will find out your predominant temperament type. \
You may be Sanguine, Choleric, Phlegmatic or Melancholic. \
You will be able to better know your own Self. \
...
and in the work team.\n")
    ...
    while True:
        ...
        if data_name not in existing_sheets:
            ...
            print(f"\n{RED}A user named '{data_name}' already exists \
in the database. Please choose a different name.{RESET}")
            continue
...
Please enter only letters.{RESET}"
        ...
        if len(values) < 3 or len(values) > 10:
            raise ValueError(
                f"{RED}the length of your name should not be \
less than 3 characters and not exceed 10 characters. \
You entered: {len(values)} characters,{RESET}"
    ...
    description_instructions = (f"  {GREEN}{values}{RESET}, you are asked \
to answer 57 questions. The questions are aimed at identifying your \
usual way of behavior. Try to imagine typical situations and give \
the first “natural” answer that comes to mind. \
If you agree with the statement, indicate {GREEN}'Yes'{RESET}, \
if not, indicate {GREEN}'No'{RESET}.\n")
...
def psychological_test(questions):
    ...
    """
    target_question_extra_intro_yes = [1, 3, 8, 10, 13, 17, 22, 25, 27,
    39, 44, 46, 49, 53, 56]
    ...
    for idx, question in enumerate(questions, start=1):
        while True:
            answer = input(f"\n{BLUE}Question № {idx}{RESET} : {question} \
            ({GREEN}Y/N){RESET}")
    ...
    temperament_type = determine_temperament(
        resalts_extra_intro,
        resalts_neuroticism
    )
    description_temperament_type = describe_temperament(
        resalts_extra_intro,
        temperament_type
    )
    description_extra_intro = describe_introversion_extroversion(
        resalts_extra_intro
    )
    description_resalts_neuroticism = describe_neuroticism(
        resalts_neuroticism
    )
    description_scale_lies, sign_profile = check_honesty(
        resalts_scale_lies
    )
...
def determine_temperament(resalts_extra_intro, resalts_neuroticism):
    ...
    intro_text = (f"\n  Your predominant temperament type is \
{YELLOW}{temperament_type}{RESET}")
...
def describe_temperament(resalts_extra_intro, temperament_type):
    ...
    if temperament_type == 'Melancholic':
        description_temperament_type = (f"\n{YELLOW} Melancholic (weak, \
unbalanced) {RESET} - the owner of a slightly inhibited reaction. Usually \
these are indecisive, closed people, prone to deep feelings. They can \
easily and steadfastly solve life's problems. On the negative side, \
a melancholic can be fearful, squeamish, concentrating on minor events \
and getting upset because of them.\n")
    elif temperament_type == 'Phlegmatic':
        description_temperament_type = (f"\n{YELLOW} Phlegmatic \
(strong, inert) {RESET} has a low level of activity. He is calm, prudent, \
able to bring the work he has begun to the end. As a rule, he treats his \
forces economically and does not waste them on unnecessary activities or \
on those that he considers so. Negative manifestations: lethargy, apathy, \
lack of will, weakly expressed emotional indicators. Others may seem boring \
and callous.\n")
    elif temperament_type == 'Sanguine':
        description_temperament_type = (f"\n{YELLOW} Sanguine{RESET} \
- the person is sociable, cheerful, easily makes new acquaintances. \
Such people are also called the soul of the company. His feelings are \
unstable, and preferences often change. He is characterized by expressive \
gestures and facial expressions. He constantly needs vivid impressions. \
In rare cases, he plans his day, spontaneity haunts the sanguine throughout \
his life in almost all areas. According to the main properties of the central \
nervous system, it has a strong and balanced character.\n")
    elif temperament_type == 'Choleric':
        description_temperament_type = (f"\n{YELLOW} Choleric (an unbalanced, \
strong type of temperament){RESET} is energetic, his actions are \
characterized by discontinuity. They can be harsh and emotional. Due to \
excessive enthusiasm for any business, they act too diligently, as a result \
of which they are quickly exhausted and tired. At its worst, the choleric \
becomes irritable and unable to control himself.\n")
    else:
...
def describe_introversion_extroversion(resalts_extra_intro):
    ...
    if resalts_extra_intro > 12:
        description_extra_intro = (" The results also showed that you are an \
extroverted personality type. This characterizes you as friendly, talkative, \
and energetic.\n")

    else:
        description_extra_intro = (" The results also showed that you are an \
introverted personality type. This manifests itself in more withdrawn and \
solitary behavior.\n")
...
def describe_neuroticism(resalts_neuroticism):
    ...
    if resalts_neuroticism <= 7:
        description_resalts_neuroticism = (" You are usually characterized \
by stable and low-intensity emotional reactions. You rarely experience \
extreme anxiety, nervousness, or depression and usually cope with stress \
better.\n")

    elif 8 < resalts_neuroticism <= 13:
        description_resalts_neuroticism = (" You usually have more stable \
emotional reactions. You may experience stress and anxiety, but not as much \
or as often as people with higher levels of neuroticism.\n")

    elif 14 < resalts_neuroticism <= 18:
        description_resalts_neuroticism = (" You tend to have emotional \
fluctuations and reactions to stress. You may experience anxiety, \
nervousness, and worry more often, but not as intensely as people \
with very high levels of neuroticism.\n")
    else:
        description_resalts_neuroticism = (" You often experience intense \
and frequent emotional reactions. You may be prone to excessive anxiety, \
fears, depression, and feelings of restlessness. You can easily fall into \
states of nervousness and uncertainty.\n")
...
def check_honesty(resalts_scale_lies):
    ...
    if resalts_scale_lies >= 5:
        description_scale_lies = (f" {RED}Important!{RESET}{GREEN} \
{data_name}{RESET}{RED}, you answered not as you really are, \
but as you would like or as accepted in society. In other words, \
your answers are not reliable.{RESET}\n")
        sign_profile = "The test subject was not sufficiently honest, \
the test results are not reliable."
    else:
        description_scale_lies = ""
        sign_profile = "The subject was honest and the test results \
are reliable."
...
def save_results_to_google_sheets(
    data_name,
    resalts_extra_intro,
    resalts_neuroticism,
    resalts_scale_lies,
    ...
    description_extra_intro,
    description_resalts_neuroticism,
    sign_profile, description_temperament_type
):
    """
...
def print_final_message(data_name):
    ...
    """
    final_message = (f"{GREEN}{data_name}, {RESET}{YELLOW}thanks for \
the answers, testing is completed!{RESET} \n \n \
To take the test again or view your results, select the desired item \
in the menu below: ")
